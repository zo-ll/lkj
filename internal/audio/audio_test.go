...
func TestRecorderForToolARecordLinux(t *testing.T) {
	rec, err := recorderForTool("linux", "arecord", "/tmp/in.wav", 1.2, "")
	if err != nil {
		t.Fatal(err)
	}
	want := recorderCommand{Name: "arecord", Args: []string{"-q", "-d", "2", "-f", "S16_LE", "-r", "16000", "-c", "1", "/tmp/in.wav"}}
...
func TestFindRecorderReturnsHelpfulError(t *testing.T) {
	_, err := findRecorder("linux", "/tmp/in.wav", 1, "", func(string) (string, error) {
		return "", errNotFound{}
	})
	if err == nil {
